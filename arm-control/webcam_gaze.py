...
def run_calibration(tracker: WebcamGazeTracker, frame_w: int, frame_h: int,
    ...
    """Calibrate a straight head pose, with optional directional stages.

    ``simple_nine_point`` + ``quick`` is main2.py's normal fixed-head
    calibration. ``full_face`` and ``relaxed_framing`` make that mode usable
    at a normal laptop-webcam distance.
    ...
    for stage_idx, (name, where, stage_targets) in enumerate(stages):
        ...
        targets = (stage_targets if head_follows else
                   grid_targets(3 if simple_nine_point else STRAIGHT_GRID_N))
