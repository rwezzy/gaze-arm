...
An experimental copy of `../arm-control-place` (look-to-place, left
untouched). The only difference: while the arm holds an object, the screen is
split into **nine regions (3 x 3)** and the place dot snaps to the center of
the region you look at; looking at one region for 1.5 s places the object at
that region's center. Selecting objects is not snapped. `gaze_snap.py` does
the snapping (a region changes only after ~4 frames there, or at once for a
big jump). Run it from this folder the same way as the others.
...
### Look-to-place (this copy only)

While holding, a steady gaze (1.5 s, within ~45 px) on **empty table**, at
least 40 px clear of every detected box, places the object there. A ring
fills while you look. The gazed pixel's camera ray is intersected with the
table (at the height the depth camera measured, ~+5 mm) while the arm is
still; the spot is refused if it is out of reach, too close to the arm base,
or within 25 mm of another object's footprint. Then it is the same checked
put-down as `P`: above the spot -> straight down to the height the object was
resting at -> open (only if the descent succeeded) -> rise -> observe pose.
Looking at another object still swaps; `P` still puts it back where it came from.
