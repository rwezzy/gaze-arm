...
async def run_session(machine: RobotClient, recovering: bool = False):
    ...
    try:
        ...
        while True:
            ...
            if held is None or MENU or gaze_pt is None or near_any_box(boxes, gaze_pt, PLACE_SPOT_BOX_MARGIN_PX):
                spot_dwell.reset()
            elif not (blinking or cooling):
                spot_anchor, spot_progress, spot = spot_dwell.update(gaze_pt, time.monotonic())
                if spot is not None:
                    feed.paused = True
                    lock.release()
            ...
            if spot_anchor is not None:
                draw_spot_dwell(view, spot_anchor, spot_progress)
