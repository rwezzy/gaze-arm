...
A development copy of `../arm-control` (the working demo, left untouched) that
adds **look-to-place**: while the arm holds an object, look at an empty spot
on the table and hold your gaze there; the arm moves to that x, y and sets the
object down. Run it from this folder the same way as the original.
